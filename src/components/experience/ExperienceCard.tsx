...
export function ExperienceCard({ entry, index }: ExperienceCardProps) {
  ...
  return (
    <Reveal3D from={index % 2 === 0 ? 'left' : 'right'} delay={index * 0.08}>
      <TiltCard
        maxTilt={4}
        lift={12}
        className={`group rounded-xl border bg-card p-6 shadow-card transition-shadow duration-300 hover:shadow-glow sm:p-7 ${
          entry.current ? 'border-primary/40' : 'border-border hover:border-primary/30'
        }`}
      >
        {entry.current ? (
          <Badge variant="solid" className="depth-3 absolute -top-3 left-6 shadow-glow">
            Current Role
          </Badge>
        ) : null}

        <div className="depth-2 flex flex-wrap items-start justify-between gap-3">
          <div>
